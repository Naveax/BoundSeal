...
mod tests {
    ...
    fn validated_low_cost_rule_is_ranked_above_noisy_expensive_rule() {
        ...
        scheduler.register_rule(profile("BSL-GOOD")).unwrap();
        scheduler.register_rule(profile("BSL-NOISY")).unwrap();
        complete_single(
            &mut scheduler,
            item(1, "BSL-GOOD", 1_000),
            observation(5, 0, 5, 0, 10),
            1,
        );
        complete_single(
            &mut scheduler,
            item(2, "BSL-NOISY", 1_000),
            observation(5, 20, 0, 5, 500),
            2,
        );
        scheduler.enqueue(item(3, "BSL-GOOD", 1_000), 3).unwrap();
        scheduler
            .enqueue(item(4, "BSL-NOISY", 1_000), 3)
            .unwrap();
        let ranking = scheduler.ranking_snapshot(3).unwrap();
        assert_eq!(ranking[0].pair.rule_id, "BSL-GOOD");
    ...
    fn unseen_rule_receives_deterministic_exploration_priority() {
        ...
        scheduler.register_rule(profile("BSL-OLD")).unwrap();
        scheduler.register_rule(profile("BSL-UNSEEN")).unwrap();
        for index in 0..10 {
            complete_single(
                &mut scheduler,
                item(index, "BSL-OLD", 10_000),
                observation(0, 0, 0, 0, 100),
                index as i64 + 1,
            );
        }
        scheduler
            .enqueue(item(100, "BSL-OLD", 10_000), 20)
            .unwrap();
        scheduler
            .enqueue(item(101, "BSL-UNSEEN", 10_000), 20)
            .unwrap();
        let ranking = scheduler.ranking_snapshot(20).unwrap();
        assert_eq!(ranking[0].pair.rule_id, "BSL-UNSEEN");
    }

    #[test]
    fn run_stop_blocks_new_work_and_leases() {
        let mut scheduler = AdaptiveRuleScheduler::new(limits(4)).unwrap();
        scheduler.register_rule(profile("BSL-RULE")).unwrap();
        scheduler.enqueue(item(1, "BSL-RULE", 1_000), 1).unwrap();
        ...
        assert_eq!(
            scheduler.enqueue(item(2, "BSL-RULE", 1_000), 2),
            Err(SchedulerError::RunStopped(RunStopReason::Saturated))
        );
    }
    ...
    fn expired_authorization_is_removed_with_reservation_release() {
        let mut scheduler = AdaptiveRuleScheduler::new(limits(4)).unwrap();
        scheduler.register_rule(profile("BSL-RULE")).unwrap();
        let pair = item(1, "BSL-RULE", 100).pair;
        scheduler.enqueue(item(1, "BSL-RULE", 100), 1).unwrap();
    ...
    fn reservation_failure_is_transactional() {
        ...
        scheduler.register_rule(profile("BSL-RULE")).unwrap();
        let mut oversized = item(1, "BSL-RULE", 1_000);
    ...
    fn ten_thousand_authorized_pairs_rank_without_a_fixed_256_ceiling() {
        ...
        for index in 0..10_000 {
            scheduler
                .enqueue(item(index, "BSL-RULE", 100_000), 1)
                .unwrap();
        }
        let ranking = scheduler.ranking_snapshot(1).unwrap();
        assert_eq!(ranking.len(), 10_000);
        assert_eq!(ranking[0].pair, item(0, "BSL-RULE", 100_000).pair);
        let receipt = scheduler.receipt().unwrap();
    ...
    fn ranking_and_receipt_are_insertion_order_independent() {
        ...
        first.register_rule(profile("BSL-A")).unwrap();
        first.register_rule(profile("BSL-B")).unwrap();
        first.enqueue(item(1, "BSL-A", 1_000), 1).unwrap();
        first.enqueue(item(2, "BSL-B", 1_000), 1).unwrap();

        let mut second = AdaptiveRuleScheduler::new(limits(8)).unwrap();
        second.register_rule(profile("BSL-B")).unwrap();
        second.register_rule(profile("BSL-A")).unwrap();
        second.enqueue(item(2, "BSL-B", 1_000), 1).unwrap();
        second.enqueue(item(1, "BSL-A", 1_000), 1).unwrap();
    ...
    fn lease_is_exactly_once_and_updates_only_its_rule_metrics() {
        let mut scheduler = AdaptiveRuleScheduler::new(limits(4)).unwrap();
        scheduler.register_rule(profile("BSL-A")).unwrap();
        scheduler.enqueue(item(1, "BSL-A", 1_000), 1).unwrap();
        ...
        let metrics = scheduler.metrics("BSL-A").unwrap();
