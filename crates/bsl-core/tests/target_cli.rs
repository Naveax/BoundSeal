...
fn init_workspace(path: &Path) {
    let output = run(&[
        "workspace", "init", "--workspace", path.to_str().unwrap(), "--name", "Target CLI Test", "--json",
    ]);
...
fn create_target(root: &Path, policy: &Path, authorization: &Path) -> Output {
    run(&[
        ...
        "--authorization-document", authorization.to_str().unwrap(),
...
fn target_lifecycle_is_networkless_and_redacted() {
    ...
    assert!(create.status.success(), "target create failed: {}", String::from_utf8_lossy(&create.stderr));
    let created: Value = serde_json::from_slice(&create.stdout).unwrap();
    assert_eq!(created.get("network_activity").and_then(Value::as_str), Some("none"));
    ...
    assert!(validate.status.success(), "target validate failed: {}", String::from_utf8_lossy(&validate.stderr));
    let validated: Value = serde_json::from_slice(&validate.stdout).unwrap();
    assert_eq!(validated.pointer("/validation/status").and_then(Value::as_str), Some("valid"));

    let list = run(&[
        "target", "list", "--workspace", root.to_str().unwrap(), "--json",
    ]);
    ...
    let show = run(&[
        "target", "show", "--workspace", root.to_str().unwrap(), "--id", "example-app", "--json",
    ]);
    assert!(show.status.success());
    let shown: Value = serde_json::from_slice(&show.stdout).unwrap();
    assert_eq!(shown.get("policy_sha256").and_then(Value::as_str), Some(sha256(&fs::read(&policy).unwrap()).as_str()));
...
fn target_failures_use_stable_exit_codes_and_json_diagnostics() {
    ...
    let unsafe_origin = run(&[
        "target", "create", "--workspace", root.to_str().unwrap(), "--id", "unsafe-origin",
        "--name", "Unsafe", "--origin", "https://user@example.org",
        "--authorization-reference", "hackerone/program/example#scope-2026",
        "--authorization-document", authorization.to_str().unwrap(), "--policy", policy.to_str().unwrap(), "--json",
    ]);
    assert_json_error(&unsafe_origin, 50, "BSL151-TARGET-CREATE-REJECTED");
    let missing = run(&[
        "target", "show", "--workspace", root.to_str().unwrap(), "--id", "missing-target", "--json",
    ]);
