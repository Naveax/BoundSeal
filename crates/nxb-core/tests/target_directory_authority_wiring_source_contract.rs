...
fn target_readiness_and_target_records_resolve_under_retained_authorities() {
    ...
    assert!(
        ...
        "{TARGET_PATH}: public workspace output must derive from the logical authority path"
    );
    assert!(
        !list.contains("workspace: root") && !list.contains("workspace: root."),
        "{TARGET_PATH}: public workspace output must not serialize the retained Linux /proc/self/fd authority path"
    );
}
