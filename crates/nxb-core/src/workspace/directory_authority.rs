...
///
/// `stable_path` is the only path intended for child I/O after admission:
/// Linux derives it from the held directory descriptor through `/proc/self/fd`,
/// while Windows keeps the canonical pathname stable by retaining a no-delete-
/// share handle chain for every admitted ancestor.
...
impl DirectoryAuthority {
    ...
    /// Stable authority-derived path for filesystem I/O.
    pub(crate) fn path(&self) -> &Path {
        &self.stable_path
    }

    /// Canonical admitted path for diagnostics/output only.
    ...
    #[cfg(target_os = "linux")]
    fn sync_unix(&self, label: &str) -> Result<()> {
    ...
    fn sync_unix(&self, _label: &str) -> Result<()> {
...
fn pin_private_directory(path: &Path, label: &str) -> Result<DirectoryAuthority> {
    ...
    super::reject_path_indirections(path, label)?;
    let canonical = fs::canonicalize(path)
        .with_context(|| format!("could not canonicalize {label} {}", path.display()))?;
    super::reject_path_indirections(&canonical, label)?;
    ...
    validate_private_unix_directory(&opened, label)?;
...
#[cfg(target_os = "linux")]
fn validate_private_unix_directory(metadata: &fs::Metadata, label: &str) -> Result<()> {
...
fn pin_private_directory(path: &Path, label: &str) -> Result<DirectoryAuthority> {
    ...
    super::validate_private_permissions(&canonical, true)?;
...
fn pin_private_child(
    ...
    if !opened.is_dir() || super::windows::is_reparse_point(&opened) {
        bail!("pinned {label} child is a reparse point or non-directory");
    }

    super::validate_private_permissions(&candidate, true)?;
...
mod windows_tests {
    ...
    fn pinned_windows_child_denies_rename_until_child_authority_is_released() {
        ...
        let targets = root.join("targets");
        fs::create_dir(&targets).unwrap();
        super::super::set_private_directory_permissions(&targets).unwrap();
