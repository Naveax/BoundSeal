...
use rustix::{
    fs::{
        linkat, open, openat, renameat_with, AtFlags, Mode, OFlags, RenameFlags, CWD,
    },
    io::Errno,
};
...
impl ParentAuthority {
    ...
    fn quarantine_no_replace(&self, source: &OsStr, quarantine: &OsStr) -> Result<()> {
        ...
        .context("could not atomically quarantine the admitted replacement destination")
    }
    ...
    fn claim_prepared(
        ...
        .context("could not create exact prepared replacement destination")?;
        self.validate_child_binding(destination, prepared, "replacement destination")
    }
}
