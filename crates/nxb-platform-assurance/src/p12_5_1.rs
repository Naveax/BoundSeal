...
pub struct FinalAssuranceCertificate {
    ...
    pub coverage_matrix_sha256: String,
    pub freeze_manifest_sha256: String,
    pub mandatory_requirement_count: usize,
    pub authority_audit_tail_hash: String,
    pub certificate_sha256: String,
}
impl FinalAssuranceCertificate {
    pub fn verify(&self) -> Result<(), AssuranceError> {
        let expected = hash_serializable(&(
            ...
            &self.coverage_matrix_sha256,
            &self.freeze_manifest_sha256,
            self.mandatory_requirement_count,
            &self.authority_audit_tail_hash,
        ))?;
        if expected != self.certificate_sha256 || self.mandatory_requirement_count == 0 {
            return Err(AssuranceError::ClosureDenied(
                "final assurance certificate".into(),
            ));
        }
        Ok(())
    }
...
#[derive(Debug)]
pub struct FinalAssuranceAuthority {
    authority_id: String,
...
impl FinalAssuranceAuthority {
    pub fn new(
        ...
        validate_identifier(&authority_id, "final assurance authority")?;
        validate_sha256(&policy_snapshot_sha256, "final assurance policy")?;
        Ok(Self {
            authority_id,
