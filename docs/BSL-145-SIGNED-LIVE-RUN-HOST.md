# BSL-145 — Signed live-run launch host

BSL-145 binds the verified BSL-139 external session, BSL-141 unified operator, BSL-143 request transaction runtime and BSL-144 resumable queue into one supervised live-run launch boundary.
...
BSL-145 adopts an already provisioned `ProvisionedExternalSession` from BSL-139. It does not silently fetch a replacement session because the BSL-141 plan binds the exact bootstrap receipt, session ID and secret-handle root.
...
For each exact BSL-144 queue head:
...
5. BSL-144 invokes the BSL-143 authenticated request transaction;
...
The `bsl-live-run-host` binary is networkless. It can build and verify launch bundles, create activation payload templates, verify signed activation certificates and consume launch activations. Actual live execution uses the library API and an explicitly supplied `LiveDnsResolver` implementation.

## Explicit limitations

BSL-145 does not discover credentials, import browser cookies, broaden scope, execute active probes, follow redirects, issue destructive methods, persist response bodies, retry indeterminate requests or submit reports. It does not reconstruct authenticated provider/session state after process loss.
