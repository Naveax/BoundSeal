...
The purpose is to prevent a rerun, concurrent reviewer or stale preparation step from silently rewriting evidence that was already associated with an exact Git head.
...
All three classes are now intended to be create-only for their canonical exact-head pathname.
...
`prepare-and-validate-nxb-153-windows.ps1` publishes the exact-head tooling receipt using `.NET FileMode.CreateNew`, `FileAccess.Write` and `FileShare.None`, then flushes the stream durably where supported.
...
`validate-nxb-153-windows.ps1` publishes the final exact-head Windows validation evidence with the same `FileMode.CreateNew` contract. `WriteAllText` overwrite semantics are not used for the canonical evidence pathname.
