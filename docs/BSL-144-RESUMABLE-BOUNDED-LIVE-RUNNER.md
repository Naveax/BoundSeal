# BSL-144 — Resumable bounded live runner

BSL-144 joins the BSL-137 deterministic passive discovery queue, the BSL-141 unified authenticated-operator binding and the BSL-143 checkpoint-bound request runtime. It remains a bounded defensive operator and does not introduce a generic attack engine.
...
- the exact number of requests committed by BSL-143;
...
A queue item is removed only after BSL-143 has durably written its `prepared -> outcome -> checkpoint -> commit` transaction. Runner and runtime counters must match exactly.
...
Prepared-only or otherwise indeterminate BSL-143 requests are never reconciled as successful and cannot be retried automatically. The runner permits only teardown or abort in that state.
...
The runner uses a stable-inode operating-system lock, canonical no-clobber publication and bounded checkpoint files. Before a live transaction, it reserves current runner bytes plus future checkpoint/terminal evidence through the BSL-143 runtime workspace accounting. Runtime state, runtime journal and runner evidence remain inside the signed unified workspace budget.
...
Runner completion is recorded only after the BSL-143 runtime reaches `completed` following provider/session/vault teardown. Runtime abort is mirrored as runner abort. Request counters must still match at the terminal checkpoint.

## Control plane

The `bsl-resumable-runner` binary provides networkless commands to:
...
BSL-144 does not mint program scope, DNS decisions, transport tickets, session secrets or activation signatures. It does not follow redirects, execute JavaScript, persist authenticated bodies, retry indeterminate requests, enable destructive methods, run active probes or submit reports automatically.
