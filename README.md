...
BoundSeal is a private, deterministic and scope-enforced bug bounty research platform for explicitly authorized targets.

## Current status

The implementation is complete through the **BSL-150 pinned process evidence-key provider** block. The workspace contains 49 private Rust crates spanning policy and scope enforcement, pinned live HTTPS transport, signed one-use activation, authenticated operator state, a resumable bounded runner, a signed live-run host, cryptographic run closure, an operator-reviewed manual submission boundary, create-only encrypted evidence persistence and concrete pinned-process evidence-key acquisition.

BSL-150 binds the BSL-149 provider-neutral evidence-key lifecycle to the existing BSL-140 absolute-path and SHA-256-pinned process transport. Its merge gate is exact-head Windows and Linux validation with deterministic dual-platform evidence closure; every final PR head must satisfy that gate independently.

This is not an unrestricted scanner. Live execution remains compile-time gated, explicitly acknowledged, signed, same-origin, HTTPS/443, GET/HEAD-only, sequential and resource bounded. BSL-147 remains a networkless manual handoff, BSL-148 persists only previously validated and redacted evidence records, BSL-149 acquires only the exact plan-bound sealing key through a one-fetch provider lifecycle, and BSL-150 does not bundle a provider-specific secret-store helper. None of these blocks calls HackerOne, accesses browser credentials or submits reports automatically.
...
- a BSL-150 adapter that reuses the pinned, shell-free process-provider transport;
...
GitHub-hosted Actions are disabled for this repository. BSL-150 validation is exposed through exact-head local harnesses:
...
```bash
cargo run -p bsl-core --locked -- validate-policy examples/target.policy.toml
cargo run -p bsl-core --locked -- validate-event examples/event.json
cargo run -p bsl-core --locked -- check-destination 8.8.8.8
```

Inspect repository status:

```bash
cargo run -p bsl-core --locked -- system-status
```

Run and verify the deterministic synthetic architecture demo:

```bash
cargo run -p bsl-core --locked -- demo-run --output target/bsl-demo-receipt.json
cargo run -p bsl-core --locked -- verify-demo target/bsl-demo-receipt.json
```
...
The `bsl-unified-operator` binary is networkless. It binds verified component artifacts, emits or verifies a unified plan, emits an external-signing template, verifies activation certificates and consumes an activation exactly once.

BSL-147 is a library contract for creating and verifying an immutable operator handoff. Manual submission remains a deliberate human action outside the repository. BSL-148 is a library-backed persistent store for encrypting canonical redacted `EvidenceRecord` values with externally supplied key material. BSL-149 defines the signed, provider-neutral lifecycle that obtains that key without serializing or logging key bytes. BSL-150 maps that lifecycle to the existing pinned process-provider protocol.
...
- [`docs/BSL-140-PINNED-PROCESS-VAULT-PROVIDER.md`](docs/BSL-140-PINNED-PROCESS-VAULT-PROVIDER.md)
- [`docs/BSL-141-UNIFIED-OPERATOR-CONTRACT.md`](docs/BSL-141-UNIFIED-OPERATOR-CONTRACT.md)
- [`docs/BSL-146-SIGNED-RUN-CLOSURE.md`](docs/BSL-146-SIGNED-RUN-CLOSURE.md)
- [`docs/BSL-147-SIGNED-MANUAL-SUBMISSION-HANDOFF.md`](docs/BSL-147-SIGNED-MANUAL-SUBMISSION-HANDOFF.md)
- [`docs/BSL-148-PRODUCTION-EVIDENCE-SEALER.md`](docs/BSL-148-PRODUCTION-EVIDENCE-SEALER.md)
- [`docs/BSL-149-EVIDENCE-KEY-PROVIDER-LIFECYCLE.md`](docs/BSL-149-EVIDENCE-KEY-PROVIDER-LIFECYCLE.md)
- [`docs/BSL-150-PINNED-PROCESS-EVIDENCE-KEY-PROVIDER.md`](docs/BSL-150-PINNED-PROCESS-EVIDENCE-KEY-PROVIDER.md)
