...
function Get-NxbTreeManifest {
    ...
    foreach ($line in $lines) {
        ...
        $mode, $type, $objectId, $sizeText = $fields
    ...
    foreach ($reserved in @('target', '.nxb-153-tmp', '.nxb-153-cargo-home')) {
        foreach ($entry in $manifest) {
            if ($entry.Path -ceq $reserved -or $entry.Path.StartsWith("$reserved/", [StringComparison]::Ordinal)) {
...
function Add-NxbSourceWriteDeny {
    param([Parameter(Mandatory = $true)][string]$Path)
...
function Invoke-NxbSelfTest {
    ...
    try {
        ...
        $null = Add-NxbSourceWriteDeny -Path $root
        $blocked = $false
        try {
...
foreach ($pair in @(@($AuditSha256, 'cargo-audit SHA-256'), @($DenySha256, 'cargo-deny SHA-256'), @($ExpectedCargoLockSha256, 'Cargo.lock SHA-256'))) {
    Assert-LowerSha256 -Value $pair[0] -Label $pair[1]
}
...
try {
    ...
    # Pin the snapshot root and every source directory before file verification.
    $allDirectories = @($snapshotRoot) + @(Get-ChildItem -LiteralPath $snapshotRoot -Directory -Force -Recurse | Sort-Object { $_.FullName.Length } | ForEach-Object { $_.FullName })
    ...
    $null = Add-NxbSourceWriteDeny -Path $snapshotRoot

    # Fail closed unless ordinary source mutation is actually blocked while the
    # dedicated runtime directories remain writable.
    $probeBlocked = $false
    try {
    ...
    try {
        ...
        foreach ($entry in $manifest) {
            $path = [string]$actualFiles[$entry.Path]
            $stream = $fileStreams[$manifest.IndexOf($entry)]
            ...
            if ((Get-Item -LiteralPath $path -Force).Length -ne $entry.Size) {
                Fail-Nxb "tracked source pathname size changed during validation: $($entry.Path)"
            }
        }
    }
    finally {
