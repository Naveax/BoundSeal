...
# Baseline: the temporary copy is writable before the production pin is held.
...
try {
    ...
    # Start from a hostile PATH ordering, prove it wins before canonical rebinding,
    # then prepend the pinned directory exactly as production does.
    ...
    # Prove success restoration back to the exact prior PATH state used by the test.
    $env:PATH = $hostileFirstPath
    ...
    # Prove the same restoration shape survives a deliberate failure.
    try {
        $env:PATH = ([string]$gitAuthority.Directory) + [IO.Path]::PathSeparator + $hostileFirstPath
...
# Cleanup release must make the same file and directory movable again.
...
try {
    ...
    Fail-NxbHostGitLifetimeProbe "temporary probe cleanup failed: $($_.Exception.Message)"
}
